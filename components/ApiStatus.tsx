  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm font-medium">{apiPath}</CardTitle>
        {apiStatus ? (
      ...
      <CardContent>
        <div
          className={`text-2xl font-bold ${apiStatus ? "text-green-500" : "text-red-500"}`}
        >
          {apiStatus ? "Operational" : "Degradation"}
        </div>
        <p className="text-xs text-muted-foreground">{apiReturn}</p>
      </CardContent>
    </Card>
  );
};
